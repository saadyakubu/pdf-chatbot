...
const Home = ()=>{
  ...
  const onDrop = useCallback(async (acceptedFiles) => {//const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    <main className="flex min-h-screen flex-col items-center p-24">
      ...
      <div className="mx-auto w-full items-center max-w-md py-24 flex flex-col stretch">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input className="w-full max-w-md text-black border border-gray-300 rounded shadow-xl p-2" 
          value={input}
          placeholder="Enter your prompt..." 
          onChange={handleInputChange} />

          <button disabled={isLoading} type="submit" className="py-2 border rounded-lg bg-gray-900 text-sm px-6">Submit</button>
